...
# 実測偽陽性(0/40)の不確実性を Pareto 前線へ伝播するための定数（査読 §主要懸念8/最優先5）。
# in-band で施行され 40 操作の偽陽性ベンチで棄却 0 と測った制御:
#   admission        = implicit_permission + insecure_default（同一 deny-privileged 制御）
#   NetworkPolicy    = isolation_gap
# Jeffreys 事前 Beta(0.5,0.5) + 0/40 観測 → 事後 Beta(0.5,40.5)。片側95%上限 r<=0.072。
_MEASURED_FP_TRIALS = 40
_FP_UPPER95 = 0.072
_INBAND_MEASURED_KINDS = frozenset({
    MisconfigKind.IMPLICIT_PERMISSION, MisconfigKind.INSECURE_DEFAULT,
    MisconfigKind.ISOLATION_GAP,
})


def _kind_of_control(misconfig_id: str):
    entry = mc.CATALOG.get(misconfig_id)
    return entry.kind if entry else None


def _zero_asr_cost_candidates(scenarios: list[Scenario], config_id: str,
                              fleet: list[FleetDefense], *, budget: int,
                              seeds: list[int]) -> dict:
    """残存 ASR を最小化する制御部分集合を、コスト再評価に必要な内訳付きで返す。

    ASR 表は棄却率 r・管理負荷 b に依存しないので一度だけ計算し、各条件では該当項の
    寄与だけ差し替えてコストを再評価する（O(候補数)）。剪定で最適になり得ない枝を落とす。
    各候補は (base_cost, subset, inband[(kind, default_r)], burden_total) を持つ。
    """
    controls = sorted({f.misconfig for f in fleet})
    n = len(controls)
    cost_map = _control_cost_map(fleet)
    default_r = {f.misconfig: f.max_rejection_rate for f in fleet}
    burden = {f.misconfig: f.management_burden for f in fleet}
    kind_of = {f.misconfig: _kind_of_control(f.misconfig) for f in fleet}
    table, mis_of = _per_scenario_asr_table(scenarios, config_id,
                                            budget=budget, seeds=seeds)
    subsets = []
    min_asr = 2.0
    for mask in range(1 << n):
        subset = tuple(controls[i] for i in range(n) if mask & (1 << i))
        asr = _residual_asr(table, mis_of, scenarios, frozenset(subset))
        subsets.append((asr, subset))
        if asr < min_asr:
            min_asr = asr
    zero = [s for a, s in subsets if a <= min_asr + 1e-12]
    opt_default = min(sum(cost_map[m] for m in s) for s in zero)
    wb = _COST_WEIGHTS["burden"]
    cands = []
    for s in zero:
        base = sum(cost_map[m] for m in s)
        # r in [0, 0.072] の変動は 1 制御あたり <=0.5*0.072、b の +-50% でも有界。
        # 既定コストが最適 + 0.3 を超える枝はどの条件でも最適になり得ないので剪定。
        if base > opt_default + 0.3:
            continue
        inband = [(kind_of[m], default_r[m]) for m in s
                  if kind_of[m] in _INBAND_MEASURED_KINDS]
        burden_total = sum(wb * burden[m] for m in s)
        cands.append((base, tuple(s), inband, burden_total))
    return {"min_asr": round(min_asr, 6), "candidates": cands,
            "opt_default_cost": round(opt_default, 4)}


def _optimal_under(cands, *, r_by_kind: dict, burden_factor: float = 1.0) -> tuple:
    """与えた in-band 棄却率と管理負荷スケールでのゼロ ASR 最小コスト集合 (cost, subset)。"""
    wr = _COST_WEIGHTS["rejection"]
    best_cost, best_set = float("inf"), None
    for base, subset, inband, burden_total in cands:
        cost = base + (burden_factor - 1.0) * burden_total
        for kind, def_r in inband:
            cost += wr * (r_by_kind[kind] - def_r)
        if cost < best_cost - 1e-12:
            best_cost, best_set = cost, subset
    return round(best_cost, 4), best_set


def frontier_rejection_sensitivity(scenarios: list[Scenario], config_id: str,
                                   fleet: list[FleetDefense], *, budget: int,
                                   seeds: list[int], n_boot: int = 2000,
                                   rng_seed: int = 0) -> dict:
    """偽陽性の不確実性を厳密 Pareto 最適へ伝播する（査読 §主要懸念8/科学的最優先5）。

    (a) in-band 制御の棄却率を r=0(実測点) / modeled proxy / 95%上限0.072 の 3 条件で
        最小コスト最適を再計算し、(b) 事後 Beta(0.5,40.5) からブートストラップして最適
        コストの分布・95% 区間・最適集合が不変かを報告する。加えて管理負荷 b の
        +-25/50% 感度も示す。ASR 表は r・b に依存しないので厳密最適集合は各条件で確定的。
    """
    prep = _zero_asr_cost_candidates(scenarios, config_id, fleet,
                                     budget=budget, seeds=seeds)
    cands = prep["candidates"]
    default_r_by_kind = {}
    for f in fleet:
        k = _kind_of_control(f.misconfig)
        if k in _INBAND_MEASURED_KINDS:
            default_r_by_kind[k] = f.max_rejection_rate

    def cond(rval):
        c, s = _optimal_under(cands, r_by_kind={k: rval
                                                for k in _INBAND_MEASURED_KINDS})
        return {"in_band_rejection": rval, "optimal_cost": c,
                "n_controls": len(s), "controls": list(s)}

    c_mod, s_mod = _optimal_under(cands, r_by_kind=default_r_by_kind)
    conditions = {
        "r_zero_measured_point": cond(0.0),
        "modeled_proxy": {
            "in_band_rejection": {k.value: round(default_r_by_kind[k], 3)
                                  for k in default_r_by_kind},
            "optimal_cost": c_mod, "n_controls": len(s_mod),
            "controls": list(s_mod)},
        "r_upper95_bound": cond(_FP_UPPER95),
    }

    rng = random.Random(rng_seed)
    costs, sets_seen = [], {}
    for _ in range(n_boot):
        r_adm = rng.betavariate(0.5, 0.5 + _MEASURED_FP_TRIALS)
        r_net = rng.betavariate(0.5, 0.5 + _MEASURED_FP_TRIALS)
        r_by_kind = {MisconfigKind.IMPLICIT_PERMISSION: r_adm,
                     MisconfigKind.INSECURE_DEFAULT: r_adm,
                     MisconfigKind.ISOLATION_GAP: r_net}
        c, s = _optimal_under(cands, r_by_kind=r_by_kind)
        costs.append(c)
        sets_seen[s] = sets_seen.get(s, 0) + 1
    costs.sort()
    lo = costs[int(0.025 * n_boot)]
    hi = costs[min(n_boot - 1, int(0.975 * n_boot))]

    # 管理負荷 b の感度（r は modeled proxy 固定）。最適集合と最適コストの振れを示す。
    burden_sens = {}
    for f in (0.5, 0.75, 1.25, 1.5):
        c, s = _optimal_under(cands, r_by_kind=default_r_by_kind, burden_factor=f)
        burden_sens[f"burden_x{f}"] = {"optimal_cost": c, "n_controls": len(s),
                                       "set_unchanged": tuple(s) == tuple(s_mod)}

    modal_set = max(sets_seen, key=sets_seen.get)
    return {
        "measured_fp_trials": _MEASURED_FP_TRIALS,
        "in_band_measured_kinds": sorted(k.value for k in _INBAND_MEASURED_KINDS),
        "min_asr": prep["min_asr"],
        "conditions": conditions,
        "bootstrap": {
            "posterior": "Jeffreys Beta(0.5,0.5) + 0/40 -> Beta(0.5,40.5) per in-band "
                         "control (admission, NetworkPolicy drawn independently)",
            "n_boot": n_boot,
            "optimal_cost_mean": round(_mean(costs), 4),
            "optimal_cost_ci95": [round(lo, 4), round(hi, 4)],
            "n_distinct_optimal_sets": len(sets_seen),
            "optimal_set_stable": len(sets_seen) == 1,
            "modal_optimal_n_controls": len(modal_set),
        },
        "burden_sensitivity": burden_sens,
    }
